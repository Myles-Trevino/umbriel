...
// SDR10 VRR retry: Both 10-bit formats fail their tests with VRR, XR30 passes
// without it and is the only commit.
...
// Commit failure: XR30 passes its test but its commit fails. XB30 is not
// committed; the sequence drops straight to XR24.
...
// HDR commit failure: The HDR tier makes one commit, clears the image
// description, and SDR10 commits next.
...
// Each mode spends one commit on HDR, one on SDR10, and two on SDR8.
UMBRIEL_TEST(worstCaseCommitCountIsBounded) {
  ...
  CHECK(!r.committed);
  CHECK(r.usedModeFallback);
  CHECK_EQ(m.count(Kind::Commit), size_t{8});
}
